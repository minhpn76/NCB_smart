...
export class UserProfileComponent implements OnInit {
  ...
  getListData(params) {
    ...
    this.ncbService.searchProfileUser(params).then((result) => {
      setTimeout(() => {
        ...
        this._totalSearch = body.numberOfElements;
  ...
  async exportExcel() {
    ...
    if (this._totalSearch > 1000) {
      ...
      const page = Math.ceil(this._totalSearch / search.size);
      ...
      try {
        ...
        this.isProcessLoadExcel = 0;
        this.toastr.success('Xuất dữ liệu theo trang thành công', 'Thành công.');
      } catch (error) {
        this.toastr.error('Không xuất được danh sách dữ liệu. Vui lòng liên hệ khối Công nghệ để được hỗ trợ', 'Lỗi hệ thống!');
      }
      return;
    }
  }
